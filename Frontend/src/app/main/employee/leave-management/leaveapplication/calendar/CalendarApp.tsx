...
export default function CalendarApp() {
  const [selectedEvent, setSelectedEvent] = useState<EmployeeLeaveModel | null>(
    null
  );
  ...
  useEffect(() => {
    if (currentLeaves) {
      setSelectedLabels(
        currentLeaves.map((leave) => leave.employeeLeaveType || "")
      );
  ...
  const handleDeleteEvent = async (eventId: number) => {
    try {
      ...
      setAllEvents((prevEvents) =>
        prevEvents.filter((event) => event.employeeLeaveId !== eventId)
      );
  ...
  const fetchEvents = useCallback(
    async (start: Date, end: Date) => {
      try {
        ...
        const newLabels = [
          ...new Set(newEvents.map((event) => event.name || "")),
        ];
        setSelectedLabels((prevLabels) => [
          ...new Set([...prevLabels, ...newLabels]),
        ]);
      } catch (error) {
  ...
  const handleEventContent = (arg: EventContentArg) => {
    ...
    return (
      <Box
        ...
        className={clsx(
          "flex items-center w-full rounded px-8 py-2 h-22 text-white"
        )}
      >
        <Typography className="text-md font-semibold">
          {arg.event.extendedProps.name} -
        </Typography>
        <Typography className="text-md px-4 font-extrabold">
  ...
  const toggleSelectedLabels = (leaveType: string) => {
    setSelectedLabels((prev) =>
      prev.includes(leaveType)
        ? prev.filter((label) => label !== leaveType)
        : [...prev, leaveType]
    );
  };
    ...
    () =>
      filteredEvents.map((event) => ({
        ...
        // end: event.endDate,
          ...
          new Date(event.endDate).setDate(new Date(event.endDate).getDate() + 1)
        ),
        extendedProps: {
          ...event,
        },
    ...
    <Root
      ...
      content={
        <>
          ...
          <div
            className={`${tabValue !== "Calendar View" ? "hidden" : ""} w-full`}
          >
            <FullCalendar
          ...
          <div
            className={`${tabValue !== "Summary View" ? "hidden" : ""} w-full`}
          >
            <LeaveSummary

...
import { Autocomplete, TextField, Button, Typography, Alert, ListItemIcon, MenuItem } from '@mui/material';
...
function EmployeeProject() {
	...
	const [employees, setEmployees] = useState<{ id: number; name: string; employeeNumber: number }[]>([]);
    
	const [getDepartments, { data: departmentsData }] = useLazyGetApiEmployeesDepartmentsQuery();
	const [getEmployees, { data: employeesData }] = useLazyGetApiProjectDepartmentEmployeesByDepartmentIdQuery();
	const [assignEmployee] = usePostApiProjectAssignMutation();
	const { data: projectEmployees, refetch: refetchProjectEmployees } = useGetApiProjectByProjectIdEmployeesQuery({
		projectId
	});
	const [removeEmployee] = useDeleteApiProjectByProjectIdEmployeesAndEmployeeIdMutation();
	...
	useEffect(() => {
		if (selectedDepartment) {
			getEmployees({ departmentId: selectedDepartment.id, projectId: parseInt(projectId, 10) });
		} else {
			setEmployees([]);
	...
	const RemoveEmployee = async (id: number) => {
		try {
			await removeEmployee({ projectId: parseInt(projectId, 10), employeeId: id });
			refetchProjectEmployees();
			dispatch(
				showMessage({
	...
	if (projectId === 'new') {
		return <Alert severity="info">Please Save Project Information to add employees To Project.</Alert>;
	}

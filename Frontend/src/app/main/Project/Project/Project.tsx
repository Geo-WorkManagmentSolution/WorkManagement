...
function Project() {
  ...
  if (
    _.isEmpty(form) ||
    (project &&
      parseInt(routeParams.projectId) !== project.id &&
      routeParams.projectId !== "new")
  ) {
    return <FuseLoading />;
  }
        ...
        content={
          <div className="p-16 sm:p-24 space-y-24">
            <FuseTabs value={tabValue} onChange={handleTabChange}>
              <FuseTab value="basic-project-info" label="Project Info" />
              <FuseTab
            ...
            <div className="">
              <div
                className={tabValue !== "basic-project-info" ? "hidden" : ""}
              >
                <BasicProjectInfoTab />
              </div>
              <div
                className={tabValue !== "work-order-documents" ? "hidden" : ""}
              >
                <WorkOrderDocuments />
              </div>
              <div
                className={tabValue !== "Project-Allocations" ? "hidden" : ""}
              >
                <EmployeeProject />
              </div>
            </div>
